...
import { CartState, CartAction, CartContextType, ProductDTO, CartItem } from '../types';
      ...
      if (existingItem) {
        const updatedItems = state.items.map(item =>
          item.product.id === action.product.id
            ? { ...item, quantity: Math.min(item.quantity + action.quantity, item.product.unitsInStock) }
            : item
        );
        return {
          items: updatedItems,
    ...
    case 'UPDATE_QUANTITY': {
      ...
      const newItems = state.items.map(item =>
        item.product.id === action.productId
          ? { ...item, quantity: Math.min(action.quantity, item.product.unitsInStock) }
          : item
      );
      return {
        items: newItems,
